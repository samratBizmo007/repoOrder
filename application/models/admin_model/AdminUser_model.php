...
public function apprUser($user_id,$passwd) {
 ...
 if ($u_result->num_rows() <= 0) {
    ...
    foreach ($u_result->result_array() as $u_key) {
        ...
        $sendPass=AdminUser_model::sendPass_saler($username, $email, $pass);  //------ send password to user email
        if($sendPass['status']==200){
            ...
            $response = array(
                'status' => 500,
                'status_message' => 'Mail Error: Password Sending Failed!');
        }
    }
...
public function getAdminEmail() {
    ...
    public function deleteUser($user_id) {
        ...
        if ($this->db->affected_rows()>0) {
            $response = array(
                'status' => 200,
                'status_message' => 'Seller Deleted Successfully');
        } else {
            $response = array(
                'status' => 500,
                'status_message' => 'Seller Not Deleted Successfully...!');
        } 
        return $response;
    }

    //---------FUNCTION FOR DELETE SELLER ENDS------------------//
}
